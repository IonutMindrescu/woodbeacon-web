        ...
        <div class="col-md-4">
            ...
            <div class="js-alerts">
                @foreach($alerts as $alert)
                    <div class="card card-alert" data-json='{"lat":"{{ $alert->lat }}","lng":"{{ $alert->lng }}","location":"{{ $alert->location }}"}'>
                        <h3>{{ $alert->created_at }}</h3>
                        Locatie: <strong>{{ $alert->location }}</strong><br/>
                        Sunet: <strong>{{ $alert->sound }}</strong>
                    </div>
                @endforeach
            </div>
        </div>
